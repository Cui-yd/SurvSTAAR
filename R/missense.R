missense <- function(gene_name, genofile, objNull, genes_info, variant_type = NULL,
  ...
  if (length(variant.id.gene) < 2) {
    ...
    if (is.null(dim(Geno)) | ncol(Geno) == 0) {
      ...
      result.missense = try(SurvSTAAR(Geno, MAF, MAC, objNull, annotation_phred = Anno.Int.PHRED.sub,
  ...
  if (length(variant.id.gene) < 2) {
    ...
    if (is.null(dim(Geno)) | ncol(Geno) == 0) {
      ...
      result.dmissense = try(SurvSTAAR(Geno, MAF, MAC, objNull, annotation_phred = Anno.Int.PHRED.sub,
                                       rare_maf_cutoff, rare_num_cutoff, combine_ultra_rare, ultra_rare_mac_cutoff,
                                       use_SPA, SPA_filter, SPA_filter_cutoff, verbose), silent = FALSE)
  ...
  if (!is.na(result.missense$SurvSTAAR_O) & !is.na(result.dmissense$SurvSTAAR_O)) {
    ...
    colnames(result.missense$SurvSTAAR_pvalue)[ncol(result.missense$SurvSTAAR_pvalue)] = "results_STAAR"
