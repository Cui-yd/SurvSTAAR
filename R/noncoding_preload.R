noncoding_preload <- function(gene_name, genofile, objNull, variant_type=c("SNV","Indel","variant"), genes_info,
  ...
  if (length(variant.is.in) < 2) {
    ...
    result.upstream = list("SurvSTAAR_O" = NA)
  } else if (rm_long && length(variant.is.in) > rm_long_cutoff) {
    ...
    if (is.null(dim(Geno)) | ncol(Geno) == 0) {
      ...
      result.upstream = try(SurvSTAAR(Geno, MAF, MAC, objNull, annotation_phred = Anno.Int.PHRED.sub,
                                      rare_maf_cutoff, rare_num_cutoff, combine_ultra_rare, ultra_rare_mac_cutoff,
                                      use_SPA, SPA_filter, SPA_filter_cutoff, verbose), silent = FALSE)
  ...
  if (length(variant.is.in) < 2) {
    ...
    result.utr = list("SurvSTAAR_O" = NA)
  } else if (rm_long && length(variant.is.in) > rm_long_cutoff) {
    ...
    if (is.null(dim(Geno)) | ncol(Geno) == 0) {
      ...
      result.utr = try(SurvSTAAR(Geno, MAF, MAC, objNull, annotation_phred = Anno.Int.PHRED.sub,
                                 rare_maf_cutoff, rare_num_cutoff, combine_ultra_rare, ultra_rare_mac_cutoff,
                                 use_SPA, SPA_filter, SPA_filter_cutoff, verbose), silent = FALSE)
  ...
  if (length(variant.is.in) < 2) {
    ...
    result.promoter.cage = list("SurvSTAAR_O" = NA)
  } else if (rm_long && length(variant.is.in) > rm_long_cutoff) {
    ...
    if (is.null(dim(Geno)) | ncol(Geno) == 0) {
      ...
      result.promoter.cage = try(SurvSTAAR(Geno, MAF, MAC, objNull, annotation_phred = Anno.Int.PHRED.sub,
                                           rare_maf_cutoff, rare_num_cutoff, combine_ultra_rare, ultra_rare_mac_cutoff,
                                           use_SPA, SPA_filter, SPA_filter_cutoff, verbose), silent = FALSE)
  ...
  if (length(variant.is.in) < 2) {
    ...
    result.promoter.dhs = list("SurvSTAAR_O" = NA)
  } else if (rm_long && length(variant.is.in) > rm_long_cutoff) {
    ...
    if (is.null(dim(Geno)) | ncol(Geno) == 0) {
      ...
      result.promoter.dhs = try(SurvSTAAR(Geno, MAF, MAC, objNull, annotation_phred = Anno.Int.PHRED.sub,
                                          rare_maf_cutoff, rare_num_cutoff, combine_ultra_rare, ultra_rare_mac_cutoff,
                                          use_SPA, SPA_filter, SPA_filter_cutoff, verbose), silent = FALSE)
  ...
  if (length(variant.is.in) < 2) {
    ...
    result.enhancer.cage = list("SurvSTAAR_O" = NA)
  } else if (rm_long && length(variant.is.in) > rm_long_cutoff) {
    ...
    if (is.null(dim(Geno)) | ncol(Geno) == 0) {
      ...
      result.enhancer.cage = try(SurvSTAAR(Geno, MAF, MAC, objNull, annotation_phred = Anno.Int.PHRED.sub,
                                           rare_maf_cutoff, rare_num_cutoff, combine_ultra_rare, ultra_rare_mac_cutoff,
                                           use_SPA, SPA_filter, SPA_filter_cutoff, verbose), silent = FALSE)
  ...
  if (length(variant.is.in) < 2) {
    ...
    result.enhancer.dhs = list("SurvSTAAR_O" = NA)
  } else if (rm_long && length(variant.is.in) > rm_long_cutoff) {
    ...
    if (is.null(dim(Geno)) | ncol(Geno) == 0) {
      ...
      result.enhancer.dhs = try(SurvSTAAR(Geno, MAF, MAC, objNull, annotation_phred = Anno.Int.PHRED.sub,
                                          rare_maf_cutoff, rare_num_cutoff, combine_ultra_rare, ultra_rare_mac_cutoff,
                                          use_SPA, SPA_filter, SPA_filter_cutoff, verbose), silent = FALSE)
  ...
  SurvSTAAR_O_all = c("upstream" = result.upstream$SurvSTAAR_O, "downstream" = result.downstream$SurvSTAAR_O,
                      "UTR" = result.utr$SurvSTAAR_O,
                      "promoter_CAGE" = result.promoter.cage$SurvSTAAR_O, "promoter_DHS" = result.promoter.dhs$SurvSTAAR_O,
                      "enhancer_CAGE" = result.enhancer.cage$SurvSTAAR_O, "enhancer_DHS" = result.enhancer.dhs$SurvSTAAR_O)
