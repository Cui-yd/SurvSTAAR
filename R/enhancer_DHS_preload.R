enhancer_DHS_preload <- function(gene_name, genofile, objNull, variant_type=c("SNV","Indel","variant"), genes_info,
  ...
  if (verbose) print(paste0(">>> Analyzing enhancer_DHS ...        ", Sys.time(), " <<<"))
  ...
  if (length(variant.is.in) < 2) {
    message("Variants number of *enhancer_DHS* is less than 2, will skip this category...")
    result.enhancer_DHS = list("SurvSTAAR_O" = NA)
  } else if (rm_long && length(variant.is.in) > rm_long_cutoff) {
    message(paste0("Variants number of *enhancer_DHS* is more than ", rm_long_cutoff, ", will skip this category..."))
    result.enhancer_DHS = list("SurvSTAAR_O" = NA)
  } else {
    ...
    if (is.null(dim(Geno)) | ncol(Geno) == 0) {
      message("Variants number of *enhancer_DHS* is less than 1, will skip this category...")
      result.enhancer_DHS = list("SurvSTAAR_O" = NA)
    } else {
      result.enhancer_DHS = try(SurvSTAAR(Geno, MAF, MAC, objNull, annotation_phred = Anno.Int.PHRED.sub,
      ...
      if (inherits(result.enhancer_DHS, "try-error")) {
        result.enhancer_DHS = list("SurvSTAAR_O" = NA)
      }
    }
  }
  ...
  result = c(list("gene_info" = gene_info_kk, "category" = "enhancer_DHS"), result.enhancer_DHS)
