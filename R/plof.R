plof <- function(gene_name, genofile, objNull, genes_info, variant_type = NULL,
  ...
  if (length(variant.id.gene) < 2) {
    ...
    if (is.null(dim(Geno)) | ncol(Geno) == 0) {
      ...
      result.plof = try(SurvSTAAR(Geno, MAF, MAC, objNull, annotation_phred = Anno.Int.PHRED.sub,
