synonymous <- function(gene_name, genofile, objNull, genes_info, variant_type = NULL,
  ...
  if (length(variant.id.gene) < 2) {
    ...
    if (is.null(dim(Geno)) | ncol(Geno) == 0) {
      ...
      result.synonymous = try(SurvSTAAR(Geno, MAF, MAC, objNull, annotation_phred = Anno.Int.PHRED.sub,
  ...
  result = c(list("gene_info" = gene_info_kk, "category" = "synonymous"), result.synonymous)
