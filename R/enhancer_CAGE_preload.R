enhancer_CAGE_preload <- function(gene_name, genofile, objNull, variant_type=c("SNV","Indel","variant"), genes_info,
  ...
  if (verbose) print(paste0(">>> Analyzing enhancer_CAGE ...        ", Sys.time(), " <<<"))
  ...
  if (length(variant.is.in) < 2) {
    ...
    result.enhancer_CAGE = list("SurvSTAAR_O" = NA)
  } else if (rm_long && length(variant.is.in) > rm_long_cutoff) {
    message(paste0("Variants number of *enhancer_CAGE* is more than ", rm_long_cutoff, ", will skip this category..."))
    result.enhancer_CAGE = list("SurvSTAAR_O" = NA)
  } else {
    ...
    if (is.null(dim(Geno)) | ncol(Geno) == 0) {
      ...
      result.enhancer_CAGE = try(SurvSTAAR(Geno, MAF, MAC, objNull, annotation_phred = Anno.Int.PHRED.sub,
  ...
  result = c(list("gene_info" = gene_info_kk, "category" = "enhancer_CAGE"), result.enhancer_CAGE)
